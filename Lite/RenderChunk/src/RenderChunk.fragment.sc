...
float renderPlaneClouds(const vec2 pos, const float rain, const float time) {
    vec2 p = pos * 2.0;
    p += time * 0.15;
...
vec2 renderClouds(const vec3 pos, const vec3 sunMoonPos, const float rain, const float time) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        clouds += renderPlaneClouds(cloudPos, rain, time);
...
float getWaterWaves(vec2 p, const float time) {
    ...
    r += snoise(vec2(p.x * 1.4 - time * 0.4, p.y * 0.65 + time * 0.4) * 0.6) * 3.5;
